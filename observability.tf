...
# -----------------------------------------------------------------------------
# Dashboard 7: Costos y Control de Gasto
# -----------------------------------------------------------------------------

resource "aws_cloudwatch_dashboard" "costs" {
  dashboard_name = "${var.project_name}-costos-${var.environment}"
    ...
    widgets = [
      {
        ...
        x      = 0, y = 0, width = 24, height = 2
        properties = {
          markdown = "# 💰 Costos y Control de Gasto\nPresupuestos por servicio, gasto acumulado y acciones de remediacion"
        }
      },
      {
        type   = "metric"
      ...
      },
      {
        ...
        x      = 0, y = 8, width = 8, height = 6
        properties = {
          title   = "Redshift - RPU-Horas Consumidas"
          region  = var.aws_region
          metrics = [
            ["AWS/Redshift-Serverless", "ComputeSeconds", "Workgroup", "${var.project_name}-wg-${var.environment}"]
          ]
          period = 3600
          stat   = "Sum"
        }
      },
      {
        ...
        x      = 8, y = 8, width = 8, height = 6
        properties = {
          title   = "Lambda - Invocaciones (proxy de costo)"
          region  = var.aws_region
          metrics = [
            ["AWS/Lambda", "Invocations", "FunctionName", "${var.project_name}-landing-processor-${var.environment}"],
            ["AWS/Lambda", "Invocations", "FunctionName", "${var.project_name}-serving-gold-${var.environment}"],
      ...
      },
      {
        ...
        x      = 16, y = 8, width = 8, height = 6
        properties = {
          title   = "Athena - TB Escaneados (proxy de costo)"
          region  = var.aws_region
          metrics = [
            ["AWS/Athena", "ProcessedBytes", "WorkGroup", "${var.project_name}-${var.environment}"]
          ]
      ...
      },
      {
        ...
        properties = {
          title   = "Bedrock - Tokens Consumidos (proxy de costo)"
          region  = var.aws_region
          metrics = [
            ["AWS/Bedrock", "InputTokenCount", "ModelId", "anthropic.claude-3-sonnet-20240229-v1:0"],
            ["AWS/Bedrock", "OutputTokenCount", "ModelId", "anthropic.claude-3-sonnet-20240229-v1:0"]
      ...
      },
      {
        ...
        properties = {
          title   = "Remediacion - Invocaciones Lambda Cost Control"
          region  = var.aws_region
          metrics = [
      ...
      {
        type   = "text"
        x      = 0, y = 20, width = 24, height = 3
        properties = {
          markdown = "## Presupuestos Configurados\n| Servicio | Budget Mensual | Alerta 80% | Alerta 100% |\n|----------|---------------|------------|-------------|\n| **Total** | $${var.budget_total} | $${tonumber(var.budget_total) * 0.8} | $${var.budget_total} |\n| **Redshift** | $${var.budget_redshift} | $${tonumber(var.budget_redshift) * 0.8} | $${var.budget_redshift} |\n| **Lambda+API** | $${var.budget_lambda_api} | $${tonumber(var.budget_lambda_api) * 0.8} | $${var.budget_lambda_api} |\n| **Storage (S3)** | $${var.budget_storage} | $${tonumber(var.budget_storage) * 0.8} | $${var.budget_storage} |"
        }
      }
    ]
  })
